---
layout: page
title: About
permalink: /about/
---

Hi, I'm **yuvicc**, a Bitcoin Core developer passionate about building robust and decentralized systems.

## What I Do

I'm actively contributing to Bitcoin Core, with a focus on:

- **[libbitcoinkernel](https://github.com/orgs/bitcoin/projects/3)** - Working on the modular library extraction project to make Bitcoin Core's consensus engine reusable
- **[java-bitcoinkernel](https://github.com/yuvicc/java-bitcoinkernel)** - Maintaining Java language bindings for libbitcoinkernel
- **Code Review** - Reviewing pull requests related to P2P networking, functional tests, and libbitcoinkernel

## Experience

### Summer of Bitcoin

- **2025** - Worked on [**bitcoinfuzz**](https://github.com/bitcoinfuzz) under the mentorship of [brunoerg](https://github.com/brunoerg)
- **2024** - Worked on [**peer-observer**](https://github.com/0xB10C/peer-observer) under the mentorship of [0xB10C](https://github.com/0xB10C)

## Education

Graduated in 2025 with a Bachelor of Technology (BTech) from [**National Institute of Technology Durgapur**.](https://nitdgp.ac.in/)

## Connect

- **GitHub**: [github.com/yuvicc](https://github.com/yuvicc)
- **Twitter**: [@hey_yuvicc](https://twitter.com/hey_yuvicc)

Feel free to reach out if you want to discuss Bitcoin development, libbitcoinkernel, or collaborate on Bitcoin open-source projects!
